...
def generate_batch(prompts, model, tokenizer, max_new_tokens):
    ...
    outputs = model.generate(**inputs, max_new_tokens=max_new_tokens, pad_token_id=tokenizer.eos_token_id, do_sample=True, temperature=0.6, top_p=0.95)
...
def main(args):
    ...
    problems = [b['prompt'] for b in benchmark]

    outputs = []
    # initial round
    template_initial = """Answer the following questions. You should think step-by-step and put your final answer within $\\boxed{{answer}}$, where [answer] is just the final number or expression that solves the problem.\n Question: {problem}"""
    template_prompts = [prompt_R1(template_initial.format(problem=problem), tokenizer) for problem in problems]
    ...
    # subsequent rounds
    template_round = """Answer the following questions. You should think step-by-step and put your final answer within $\\boxed{{answer}}$, where [answer] is just the final number or expression that solves the problem.\n Question: {problem}\nThe assistant's previous answer is: {pre_answer}, and please re-answer."""
    for round in range(1, args.max_n):
        prompts = []
        pre_answers = []
        for j, item in enumerate(outputs):
            last_round_ans = item['outputs'][-1]
            pre_answer = clean_output(last_round_ans)
            pre_answers.append(pre_answer)
            pre_answer = f"$\\boxed{{{pre_answer}}}$"
            prompts.append(prompt_R1(template_round.format(problem=item['problem'], pre_answer=pre_answer), tokenizer))
        responses = generate_batch_vllm(prompts, model, sampling_params)
        for j, response in enumerate(responses):
            outputs[j]['outputs'].append(response)
            outputs[j]['pre_answers'].append(pre_answers[j])
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Run sequential inference for an LLM')
