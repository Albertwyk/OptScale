...
def convert_csv_to_latex(csv_file):
    """
    Convert CSV results table to LaTeX format.
    
    Includes all baseline methods:
    - Best-of-N (BoN)
    - Self-Consistency
    - MR-Thinking
    ...
    # Remove empty rows and rows with "OptScale (Ours)"
    # This keeps all baseline methods including:
    # - Best-of-N (BoN), Self-Consistency, MR-Thinking
    # - Early-stopping SC (ESC), Adaptive SC (ASC), Difficulty-Adaptive SC (DSC)
    # - OptScale by MLE (Ours), OptScale w/ Predictor (Ours)
    df = df.dropna(subset=['Baseline'])
    df = df[~df['Baseline'].str.contains('OptScale \\(Ours\\)', na=False)]
...
def main():
    ...
    try:
        ...
        output_file = 'latex_table_final_output.txt'
